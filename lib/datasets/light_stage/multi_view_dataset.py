...
class Dataset(data.Dataset):
    def __init__(self, data_root, human, ann_file, split):
        ...
        test_view = [i for i in range(num_cams) if i not in cfg.training_view]      # 剩余的全部是测试视角
    ...
    def prepare_input(self, i):
        ...
        vertices_path = os.path.join(self.data_root, cfg.vertices,
                                     '{}.npy'.format(i))
        xyz = np.load(vertices_path).astype(np.float32)     # smpl vertices world coords (6890,3)
        ...
        if cfg.big_box:
            ...
            min_xyz[2] -= 0.05
            max_xyz[2] += 0.05
        can_bounds = np.stack([min_xyz, max_xyz], axis=0)   # bbox
        ...
        xyz = np.dot(xyz - Th, R)                   # smpl 顶点位置 去除了全局RT
        ...
        dhw = xyz[:, [2, 1, 0]]             # xyz -> zyx
        min_dhw = min_xyz[[2, 1, 0]]
        max_dhw = max_xyz[[2, 1, 0]]
        voxel_size = np.array(cfg.voxel_size)
        coord = np.round((dhw - min_dhw) / voxel_size).astype(np.int32)

        # construct the output shape(bbox内有多少voxel，round到32的倍数)
