...
class Renderer:
    ...
    def get_pixel_value(self, ray_o, ray_d, near, far, feature_volume,
        ...
        ray_d: (batch, num_pixel, 3)'''
        # sampling points along camera rays         
        wpts, z_vals = self.get_sampling_points(ray_o, ray_d, near, far)        # 世界坐标系下的采样点位置和采样深度
        # wpts: (batch, num_pixels. num_sampling, 3)  z_vals: (batch, num_pixels, num_sampling)

        # viewing direction         归一化viewdir
